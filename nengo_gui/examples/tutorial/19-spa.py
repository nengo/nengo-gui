...
# numerical values such that concepts with similar semantics (like DOG and CAT)
# might have similar numerical values.

# To help work with these vectors, we introduce a new collection of pre-built
# Networks, and a new type of graph.  The new pre-built Networks can be accessed
# via nengo.spa ("spa" stands for "Semantic Pointer Architecture", a collection
# of tools for making these sorts of models.  A spa.Buffer is simply a Network
# that holds one vector.  A spa.Memory can hold one vector but also includes
# a recurrent feedback connection that means it will remember that value
# even after its input has been taken away.
#
...
with model:
    ...
    nengo.Connection(model.vision.state.output, model.memory.state.input,
                        transform=0.1)