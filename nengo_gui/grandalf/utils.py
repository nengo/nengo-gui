...
def rand_ortho1(n):
    ...
    # inf X:\grandalf\grandalf\layouts.py:784:
...
def intersectR(view, topt):
    ...
    # inside the bb !
    raise ValueError(
        'no intersection found (point inside ?!). view: %s topt: %s'
        % (view, topt))
...
def setroundcorner(e, pts):
    ...
    while k < n:
        ...
        if (k + 1) < n:
            splines.extend(
                setcurve(e, [P[k - 1], P[k + 1]], tgs=[T[k - 1], T[k + 1]]))
        else:
...
class Dot:
    ...
    def read(self, filename):
        # As it'll try to decode later on with utf-8, read it binary at this
        # point.
        f = file(filename, 'rb')
