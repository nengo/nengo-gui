...
class VertexViewer(object):
    ...
    def __str__(self, *args, **kwargs):
        if hasattr(self, 'xy'):
            return 'VertexViewer (xy: %s) w: %s h: %s' % (
...
class SugiyamaLayout(object):
    ...
    def dirvh(self, dirvh):
        ...
        self.__dirvh = dirvh
        self.__dirh, self.__dirv = {
            0: (1, -1), 1: (-1, -1), 2: (1, 1), 3: (-1, 1)}[dirvh]
    ...
    def rank_all(self, roots, optimize=False):
        self._edge_inverter()
        r = list(filter(lambda x: len(list(x.e_in()))
                        == 0 and x not in roots, self.g.sV))
        ...
        if g[v].X is not None:
            ...
            while 1:
                ...
                if 0 <= j < len(self.layers[r]):
                    ...
                    delta = self.xspace + (wprec.view.w + w.view.w) / \
                        2.   # abs positive minimum displ.
...
class DigcoLayout(object):
    ...
    def _optimize(self, Z, limit=100):
        ...
        while count < limit:
            ...
            b = self.__Lij_Z_Z(Z)
            # update new stress:
            FZ += 2 * float(x.transpose() *
                            b[1:, 0] + y.transpose() * b[1:, 1])
            # test convergence:
            print('stress=%.10f' % FZ)
            if stress == 0.0:
                break
