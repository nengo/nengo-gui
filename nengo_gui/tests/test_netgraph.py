...
class TestComponentManager(object):
    ...
    def test_load(self, client, example, tmpdir):
        ...
        execfile(fname)
        manager = ComponentManager(client)
        manager.load("%s.cfg" % (fname,), locals()["model"], locals())
    ...
    def test_update_create(self, client):
        ...
        components = ComponentManager(client)
...
class TestLiveContext(object):
    ...
    def test_load(self, client, example, tmpdir):
        ...
        context.load(str(tmpdir.join("default.py")))
...
class TestNetGraph(object):
    ...
    def test_load(self, client, all_examples, tmpdir):
        shutil.copy2(all_examples, str(tmpdir))
        filename = str(tmpdir.join(os.path.basename(all_examples)))
        ...
        try:
            ...
            filename_cfg = "%s.cfg" % filename
        except IOError:
            print("No %s.cfg file" % all_examples)
            filename_cfg = None
