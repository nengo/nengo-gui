...
def test_upgrade(client, example, tmpdir):
    shutil.copy2(example("default.py"), str(tmpdir))
    shutil.copy2(example("default.py.cfg"), str(tmpdir))

    execfile(str(tmpdir.join("default.py")))
    with open(str(tmpdir.join("default.py.cfg")), "r") as fp:
        text = fp.read()
    print(upgrade(text, locals=locals()))
    assert False
