...
create_modal() {
    ...
    info.uid = this.uid;
    info.conn_in_uids = this.conn_in.map(function(c) {
        return c.uid;
    });
    info.conn_out_uids = this.conn_out.map(function(c) {
        return c.uid;
    });
    this.ng.notify(info);
};
 ...
 */
collapse(report_to_server, auto) {
    auto = typeof auto !== 'undefined' ? auto : false;
