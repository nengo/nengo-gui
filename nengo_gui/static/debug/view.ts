...
export class ControlGroupView {
    ...
    constructor(label: string) {
        const node = h("div.control-group.last", [
            h("div", [
            ...
            h("div", [
                h("p", [h("code", [h("span.glyphicon.glyphicon-console")])]),
                h("p", [h("code#output")])
            ])
        ]);
        this.root = dom.create(node).domNode as HTMLDivElement;
        this.input = this.root.querySelector("input") as HTMLInputElement;
        this.evalButton = this.root.querySelector("#eval") as HTMLButtonElement;
        this.evalOutput = this.root.querySelector("#output") as HTMLElement;
        this.removeButton = this.root.querySelector(
            "#remove"
        ) as HTMLButtonElement;
    }

    addConnectButton() {
        const node = h("button.btn.btn-xs.btn-default.pull-right#connect", [
...
export class DebugView {
    ...
    constructor() {
        const button = (
            ...
            return h(
                `button.btn.btn-default.btn-block${activeClass}#${id}`,
                {
                    autocomplete: "off",
                    "data-toggle": "button",
                    type: "button"
        ...
        const menu = (id: string, label: string = ""): VNode => {
            ...
            return h(`div.dropup#${id}`, [
                h(
                    "button.btn.btn-default.btn-block.dropdown-toggle",
                    {
        ...
        this.outline = this.controls.querySelector(
            "#outline"
        ) as HTMLButtonElement;
