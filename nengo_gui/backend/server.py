...
class HttpError(Exception):
    def __init__(self, code, msg, headers=(), data=None):
        ...
        if data is None:
            data = '<h1>' + str(self.code) + '</h1><p>' + msg + '</p>'
        self.data = data
...
class InternalServerError(HttpError):
    def __init__(self, msg):
        super(InternalServerError, self).__init__(
            500, 'Internal server error', data=msg)
...
class HtmlResponse(HttpResponse):
    def __init__(self, body, code=200, headers=(), encoding='utf-8'):
        data = '<html><body>' + body + '</body></html>'
        super(HtmlResponse, self).__init__(
            data.encode(encoding), code=code, headers=headers)
...
class WebSocketFrame(object):
    ...
    def parse(cls, data):
        try:
            ...
            datalen = data[1] & 0x7F
            mask = bytearray('\x00\x00\x00\x00')
    ...
    def create_text_frame(cls, text, mask=0):
        return cls(1, 0, cls.OP_TEXT, mask, text.encode('ascii'))
